...
class MatrixCell(Node):
    ...
    def matches_goal(self, goal) -> bool:
        return (self._pos[0] == self._num_rows - 1) and (self._pos[1] == self._num_cols - 1)

    # h is the heuristic function. h() estimates the cost to reach goal from this node.
    def h(self) -> int:
        return ((self._num_rows - self._pos[0]) + (self._num_cols - self._pos[1])) * self._avg_val
